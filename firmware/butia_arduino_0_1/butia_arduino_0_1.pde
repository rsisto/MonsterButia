...
#define NUM_MOTORS    2                             // numero de motores
#define CODE_ERROR    0xFF                          // codigo de error
...
byte (* handler[]) (byte* buffer, byte largo) = {sistema, butia, tortuga, motores, sensores, lback, ax12};    // funciones "handler" 

// storage class para la "máquina de estados" de recepción
enum ESTADO { BUSCAR, LEER_LARGO, LEER_MSG, CHECKSUM }; 

char* nombres [] = {"butia", "tortuga", "motores", "sensor", "lback", "ax12"};
...
void setup() {   
  ...
  pinMode (7, INPUT);                                // Defino este pin como entrada para leer sensores digitales
  pinMode (6, INPUT);
  pinMode (5, INPUT);
  pinMode (4, INPUT);
