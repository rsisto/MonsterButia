...
static void InitializeSystem(void){
	...
	mInitPortA();mInitPortB();mInitPortC();mInitPortD();mInitPortE(); // set pins as input
	initISRFunctions();
	//initT0Service(); jorge lo descomento en su proyecto, supongo que es responsabilidad del usermodule inicializarlo
	initPollingFunctions();
	initHandlerManager();
